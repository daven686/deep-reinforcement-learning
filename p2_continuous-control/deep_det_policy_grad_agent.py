...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, actor_net, critic_net):
        ...
        self.actor_local = actor_net(state_size, action_size, random_seed).to(device)
        self.actor_target = actor_net(state_size, action_size, random_seed).to(device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=LR_ACTOR)

        # Critic Network (w/ Target Network)
        self.critic_local = critic_net(state_size, action_size, random_seed).to(device)
        self.critic_target = critic_net(state_size, action_size, random_seed).to(device)
    ...
    def step(self, state, action, reward, next_state, done, step):
        ...
        # Learn, if enough samples are available in memory
        if len(self.memory) > BATCH_SIZE and (step % 20) == 0 :
            for _ in range(7) :
                experiences = self.memory.sample()
                self.learn(experiences, GAMMA)
